...
"""PyTorch BERT model. Modified to add adapters in each layer"""
...
# Change log ------------------------------------
# above copy from transformers == 4.10.2
# add adapters to each layer
# Change log ------------------------------------


import sys
from transformers.models.bert.modeling_bert import BertModel, BertSelfOutput, BertEncoder, BertOutput, BertLayer, BertAttention
from networks.adapters import BertAdapterMask, BertAdapter
...
class MyBertSelfOutput(BertSelfOutput):
    def __init__(self, config,args):
        ...
        if args.apply_bert_attention_output:
            ...
            if args.build_adapter_capsule_mask:
...
class MyBertAttention(BertAttention):
    ...
    def forward(
        ...
        if self.args.apply_bert_attention_output:
            if self.args.build_adapter_capsule_mask:
                output_dict = self.output(self_outputs[0], hidden_states, t=t, s=s)
...
class MyBertEncoder(BertEncoder):
    ...
    def compute_layer_outputs(self, output_attentions,layer_module,
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = self.compute_layer_outputs(
...
class MyBertModel(BertModel):
    ...
    def forward(
        ...
        encoder_outputs = self.compute_encoder_outputs(
                                ...
                                output_hidden_states,return_dict,t=t,s=s)
